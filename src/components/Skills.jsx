import { useState, useEffect, useRef } from "react";
import { useTranslation } from 'react-i18next';

export default function Skills() {
    const { t } = useTranslation();
    const skills = [
        {
            name: "HTML",
            desc: t("skills3"),
            value: 95,
            color: "#E44D26",
            svg: (
                <svg className="icon" viewBox="0 0 128 128" aria-hidden="true">
                    <path fill="#E44D26" d="M19 3l9 100 36 10 36-10 9-100z" />
                    <path fill="#F16529" d="M64 7v100.6l29.1-8.1 7.7-86.7z" />
                    <path fill="#EBEBEB" d="M64 66H49l-1-12h16V42H35l3 34h26z" />
                    <path fill="#fff" d="M64 66v14l13 .4 1.1-12.1zM64 42v12h27l1-12z" />
                </svg>
            ),
        },
        {
            name: "CSS",
            desc: t("skills4"),
            value: 95,
            color: "#1572B6",
            svg: (
                <svg className="icon" viewBox="0 0 128 128" aria-hidden="true">
                    <path fill="#1572B6" d="M19 3l9 100 36 10 36-10 9-100z" />
                    <path fill="#33A9DC" d="M64 7v100.6l29.1-8.1 7.7-86.7z" />
                    <path fill="#fff" d="M64 42h28l-1 12H64zm0 24h13l-1 14-12-.3z" />
                    <path fill="#EBEBEB" d="M64 66H49l-1-12h16zM64 42H35l2 12h27z" />
                </svg>
            ),
        },
        {
            name: "Sass",
            desc: t("skills5"),
            value: 70,
            color: "#CF649A",
            svg: (
                <svg className="icon" viewBox="0 0 128 128" aria-hidden="true">
                    <path fill="#CF649A" d="M64 18c-29 0-52 14-52 29 0 9 8 16 21 23 8-5 17-8 25-10-2-8 4-18 10-20 11-4 21 6 21 17 0 12-6 26-15 34-13 11-28 13-37 7-6-4-6-10-3-14 3-4 8-5 11-4-1 6-3 10 2 13 7 4 22-1 31-11 8-9 12-22 8-29-4-6-10-6-14-4-5 2-8 8-7 13-9 2-19 6-27 11-16-8-26-17-26-29C12 30 36 18 64 18z" />
                </svg>
            ),
        },
        {
            name: "Node.js",
            desc: t("skills6"),
            value: 80,
            color: "#68A063",
            svg: (
                <svg viewBox="0 0 32 32" fill="none" width="25" height="25" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M17.1725 2.29872C16.4627 1.89953 15.5373 1.90132 14.8269 2.29872C11.2689 4.26227 7.71082 6.22641 4.15216 8.18906C3.45969 8.55335 2.99264 9.29698 3.00009 10.0688V21.9328C2.99509 22.7197 3.48622 23.4705 4.19655 23.8298C5.21871 24.3736 6.2118 24.9726 7.25244 25.4802C8.45451 26.0709 9.95843 26.2015 11.1752 25.5855C12.1629 25.075 12.6016 23.9395 12.6003 22.896C12.6083 18.9806 12.6016 15.0651 12.6034 11.1496C12.6269 10.9756 12.4962 10.7896 12.3064 10.7938C11.8517 10.7866 11.3964 10.7896 10.9417 10.7926C10.7699 10.7764 10.6022 10.9191 10.6152 11.0918C10.6091 14.982 10.6164 18.8734 10.6115 22.7642C10.6214 23.3024 10.2578 23.8196 9.73913 24.0014C8.5412 24.4213 5.12198 22.2012 5.12198 22.2012C4.9965 22.1431 4.91682 22.007 4.92912 21.8718C4.92912 17.9576 4.92973 14.0433 4.92912 10.1297C4.91187 9.97191 5.00912 9.8298 5.15402 9.76538C8.70033 7.8134 12.2448 5.85654 15.7911 3.90336C15.9143 3.82115 16.086 3.8214 16.2089 3.90396C19.7552 5.85654 23.3003 7.81161 26.8472 9.76368C26.9926 9.828 27.0857 9.9725 27.0709 10.1297C27.0703 14.0433 27.0721 17.9576 27.0697 21.8713C27.0802 22.0098 27.0086 22.144 26.8793 22.2048C23.3661 24.1462 19.8129 26.025 16.3315 28.0228C16.1796 28.1099 16.0075 28.2086 15.8373 28.1126C14.9218 27.6062 14.0174 27.0801 13.1049 26.5688C13.0057 26.5069 12.8794 26.4803 12.7759 26.5496C12.3668 26.7652 11.982 26.9398 11.5122 27.1258C10.8524 27.387 10.9578 27.4938 11.5529 27.8405C12.62 28.4444 13.6889 29.0459 14.756 29.6504C15.4585 30.0888 16.4024 30.12 17.1275 29.7149C20.6861 27.7538 24.2436 25.7904 27.8029 23.8293C28.5113 23.468 29.0049 22.7202 28.9999 21.9327V10.0688C29.0068 9.31264 28.5576 8.58227 27.886 8.21259C24.3156 6.23947 20.7435 4.27064 17.1725 2.29872Z" fill="#8CC84B"></path> <path d="M22.5419 11.2062C21.1452 10.459 19.4836 10.4192 17.9315 10.5169C16.8102 10.6277 15.6309 10.9371 14.814 11.7409C13.9761 12.5489 13.7937 13.8537 14.1917 14.9085C14.4769 15.6539 15.1948 16.1386 15.9372 16.395C16.8935 16.7326 17.8979 16.837 18.9026 16.9414C19.819 17.0366 20.7357 17.1319 21.6165 17.4042C21.9763 17.5234 22.3953 17.7058 22.5055 18.0973C22.6073 18.5609 22.4957 19.0998 22.1193 19.4219C20.9237 20.3682 17.5979 20.2232 16.4166 19.4784C15.939 19.1611 15.7332 18.5994 15.6495 18.0641C15.6402 17.8973 15.5059 17.7443 15.3248 17.757C14.8713 17.7516 14.4178 17.7528 13.9643 17.7564C13.8061 17.7431 13.6416 17.8557 13.6329 18.0172C13.5397 20.4689 15.7914 21.5377 17.9039 21.773C19.1108 21.888 20.3442 21.8814 21.5327 21.6224C22.4261 21.419 23.3219 21.0444 23.9369 20.3563C24.6953 19.52 24.8444 18.2749 24.5043 17.2332C24.2443 16.4559 23.5012 15.9573 22.7416 15.7008C21.7086 15.3466 20.4844 15.1562 19.5488 15.0671C18.1889 14.9376 16.5729 14.9905 16.188 14.0969C16.0345 13.629 16.1651 13.048 16.5951 12.7602C17.7328 11.9885 20.0483 12.091 21.2265 12.6675C21.7675 12.9384 22.081 13.4948 22.2104 14.0565C22.2344 14.2215 22.3454 14.3937 22.5364 14.3865C22.9868 14.3955 23.4372 14.3889 23.8875 14.3895C24.0422 14.4003 24.2116 14.313 24.2418 14.1546C24.2227 12.9806 23.6232 11.7788 22.5419 11.2062Z" fill="#8CC84B"></path> </g></svg>
            ),
        },
        {
            name: "SCSS",
            desc: t("skills7"),
            value: 80,
            color: "#CD6799",
            svg: (
                <svg className="icon" viewBox="0 0 128 128">
                    <path fill="#CD6799" d="M64 12C33 12 12 32 12 64s21 52 52 52 52-21 52-52S95 12 64 12z" />
                    <text x="64" y="80" textAnchor="middle" fontSize="36" fill="#fff">S</text>
                </svg>
            ),
        },
        {
            name: "Laravel",
            desc: t("skills8"),
            value: 70,
            color: "#FF2D20",
            svg: (
                <svg fill="#FF2D20" viewBox="0 0 32 32" version="1.1" width="25" height="25" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>laravel</title> <path d="M13.143 23.585l10.46-5.97-4.752-2.736-10.453 6.019zM24.084 11.374l-4.757-2.736v5.417l4.758 2.737zM24.559 5.078l-4.756 2.736 4.756 2.736 4.755-2.737zM9.911 18.928l2.76-1.589v-11.934l-4.758 2.738v11.934zM7.437 1.846l-4.756 2.737 4.756 2.737 4.753-2.737zM2.204 5.406v18.452l10.464 6.022v-5.471l-5.472-3.096c-0.018-0.013-0.032-0.027-0.051-0.039-0.014-0.013-0.030-0.023-0.044-0.034l-0.001-0.003c-0.015-0.015-0.028-0.031-0.039-0.049l-0.001-0.001c-0.014-0.013-0.025-0.028-0.035-0.045l-0.001-0.001h-0.003c-0.008-0.015-0.016-0.035-0.024-0.055l-0.001-0.004c-0.007-0.015-0.015-0.032-0.022-0.051l-0.001-0.003c-0.004-0.020-0.008-0.045-0.010-0.070l-0-0.002c-0.003-0.015-0.006-0.033-0.008-0.051l-0-0.001v-12.759l-2.757-1.59zM24.085 23.857v-5.422l-10.464 5.974v5.47zM29.789 14.055v-5.417l-4.756 2.737v5.417zM30.725 7.69c0.011 0.038 0.018 0.081 0.018 0.126v0 6.513c-0 0.176-0.095 0.329-0.237 0.411l-0.002 0.001-5.468 3.149v6.241c-0 0.175-0.095 0.328-0.236 0.411l-0.002 0.001-11.416 6.57c-0.024 0.013-0.052 0.025-0.081 0.033l-0.003 0.001-0.030 0.013c-0.036 0.011-0.078 0.017-0.121 0.017s-0.085-0.006-0.125-0.018l0.003 0.001c-0.015-0.004-0.027-0.009-0.039-0.016l0.001 0.001c-0.031-0.011-0.057-0.021-0.082-0.033l0.004 0.002-11.413-6.57c-0.144-0.084-0.239-0.237-0.239-0.412v0-19.548c0-0.044 0.007-0.087 0.019-0.127l-0.001 0.003c0.004-0.015 0.013-0.025 0.018-0.040 0.009-0.029 0.019-0.053 0.030-0.076l-0.001 0.003c0.008-0.016 0.018-0.030 0.029-0.042l-0 0 0.042-0.057 0.047-0.034c0.018-0.015 0.034-0.030 0.052-0.043h0.001l5.708-3.285c0.068-0.040 0.15-0.064 0.237-0.064s0.169 0.024 0.239 0.065l-0.002-0.001 5.71 3.285c0.019 0.013 0.035 0.027 0.051 0.042l-0-0 0.048 0.034c0.016 0.018 0.025 0.038 0.042 0.057 0.012 0.012 0.022 0.026 0.031 0.041l0.001 0.001c0.010 0.020 0.020 0.044 0.029 0.069l0.001 0.004 0.016 0.040c0.011 0.035 0.018 0.076 0.018 0.118 0 0.002 0 0.004-0 0.006v-0 12.208l4.756-2.737v-6.241c0-0.001 0-0.002 0-0.002 0-0.043 0.006-0.085 0.017-0.125l-0.001 0.003c0.004-0.013 0.013-0.025 0.016-0.040 0.010-0.030 0.020-0.054 0.032-0.078l-0.002 0.004c0.009-0.015 0.023-0.025 0.032-0.042 0.015-0.019 0.027-0.038 0.042-0.054 0.014-0.013 0.029-0.025 0.045-0.035l0.001-0.001c0.018-0.013 0.033-0.029 0.052-0.040h0.001l5.708-3.286c0.068-0.040 0.15-0.064 0.237-0.064s0.169 0.024 0.239 0.065l-0.002-0.001 5.708 3.286c0.020 0.013 0.034 0.027 0.053 0.039 0.015 0.013 0.032 0.023 0.046 0.035 0.016 0.018 0.028 0.038 0.043 0.056 0.011 0.012 0.021 0.026 0.030 0.040l0.001 0.001c0.012 0.022 0.022 0.047 0.030 0.073l0.001 0.003c0.008 0.012 0.014 0.025 0.019 0.039l0 0.001z"></path> </g></svg>
            ),
        },
        {
            name: "GitHub",
            desc: t("skills9"),
            value: 88,
            color: "#181717",
            svg: (
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-github" viewBox="0 0 16 16">
                    <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8" />
                </svg>
            ),
        },
        {
            name: "Bootstrap",
            desc: t("skills10"),
            value: 95,
            color: "#7952B3",
            svg: (
                <svg viewBox="0 0 256 256" version="1.1" width="25" height="25" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid" fill="#000000"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M0,222.991225 C0,241.223474 14.7785318,256 33.0087747,256 L222.991225,256 C241.223474,256 256,241.221468 256,222.991225 L256,33.0087747 C256,14.7765263 241.221468,0 222.991225,0 L33.0087747,0 C14.7765263,0 0,14.7785318 0,33.0087747 L0,222.991225 Z" fill="#563D7C"> </path> <path d="M106.157563,113.238095 L106.157563,76.9845938 L138.069328,76.9845938 C141.108559,76.9845938 144.039202,77.2378593 146.861345,77.7443978 C149.683488,78.2509362 152.179961,79.1554557 154.35084,80.4579832 C156.52172,81.7605107 158.258397,83.5695496 159.560924,85.8851541 C160.863452,88.2007585 161.514706,91.1675823 161.514706,94.7857143 C161.514706,101.298352 159.560944,106.001853 155.653361,108.896359 C151.745779,111.790864 146.752832,113.238095 140.67437,113.238095 L106.157563,113.238095 L106.157563,113.238095 Z M72.07493,50.5 L72.07493,205.5 L147.186975,205.5 C154.133788,205.5 160.899594,204.631661 167.484594,202.894958 C174.069594,201.158255 179.93088,198.480877 185.068627,194.862745 C190.206375,191.244613 194.294803,186.577293 197.334034,180.860644 C200.373264,175.143996 201.892857,168.37819 201.892857,160.563025 C201.892857,150.866431 199.541107,142.581033 194.837535,135.706583 C190.133963,128.832132 183.00635,124.020088 173.454482,121.270308 C180.401295,117.941627 185.647508,113.672295 189.193277,108.462185 C192.739047,103.252075 194.511905,96.7395349 194.511905,88.9243697 C194.511905,81.6881057 193.317939,75.6097352 190.929972,70.6890756 C188.542005,65.7684161 185.177193,61.8247114 180.835434,58.8578431 C176.493676,55.8909749 171.283644,53.756309 165.205182,52.4537815 C159.12672,51.151254 152.397096,50.5 145.016106,50.5 L72.07493,50.5 L72.07493,50.5 Z M106.157563,179.015406 L106.157563,136.466387 L143.279412,136.466387 C150.660401,136.466387 156.594049,138.166883 161.080532,141.567927 C165.567016,144.968971 167.810224,150.649353 167.810224,158.609244 C167.810224,162.661552 167.122789,165.990183 165.747899,168.595238 C164.373009,171.200293 162.527789,173.262597 160.212185,174.782213 C157.89658,176.301828 155.219203,177.387252 152.179972,178.038515 C149.140741,178.689779 145.956833,179.015406 142.628151,179.015406 L106.157563,179.015406 L106.157563,179.015406 Z" fill="#FFFFFF"> </path> </g> </g></svg>
            ),
        },
        {
            name: "Tailwind",
            desc: t("skills11"),
            value: 60,
            color: "#38B2AC",
            svg: (
                <svg className="icon" viewBox="0 0 128 128">
                    <circle cx="64" cy="64" r="52" fill="#38B2AC" />
                    <text x="64" y="80" textAnchor="middle" fontSize="36" fill="#fff">T</text>
                </svg>
            ),
        },
        {
            name: "PHP",
            desc: t("skills12"),
            value: 80,
            color: "#777BB4",
            svg: (
                <svg viewBox="0 0 32 32" fill="none" width="25" height="25" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <circle cx="16" cy="16" r="14" fill="#8892BF"></circle> <path d="M14.4392 10H16.1192L15.6444 12.5242H17.154C17.9819 12.5419 18.5986 12.7269 19.0045 13.0793C19.4184 13.4316 19.5402 14.1014 19.3698 15.0881L18.5541 19.4889H16.8497L17.6288 15.2863C17.7099 14.8457 17.6856 14.533 17.5558 14.348C17.426 14.163 17.146 14.0705 16.7158 14.0705L15.3644 14.0573L14.3661 19.4889H12.6861L14.4392 10Z" fill="white"></path> <path fillRule="evenodd" clipRule="evenodd" d="M6.74092 12.5243H10.0036C10.9612 12.533 11.6552 12.8327 12.0854 13.4229C12.5156 14.0132 12.6576 14.8193 12.5115 15.8414C12.4548 16.3085 12.3289 16.7665 12.1341 17.2159C11.9474 17.6652 11.6878 18.0704 11.355 18.4317C10.9491 18.8898 10.5149 19.1805 10.0523 19.304C9.58969 19.4274 9.11076 19.489 8.61575 19.489H7.15484L6.69222 22H5L6.74092 12.5243ZM7.43485 17.9956L8.16287 14.0441H8.40879C8.49815 14.0441 8.5914 14.0396 8.6888 14.0309C9.33817 14.0221 9.87774 14.0882 10.308 14.2291C10.7462 14.37 10.8923 14.9031 10.7462 15.8282C10.5678 16.9296 10.2186 17.5727 9.69926 17.7577C9.1799 17.934 8.53053 18.0176 7.75138 18.0088H7.58094C7.53224 18.0088 7.48355 18.0043 7.43485 17.9956Z" fill="white"></path> <path fillRule="evenodd" clipRule="evenodd" d="M24.4365 12.5243H21.1738L19.4329 22H21.1251L21.5878 19.489H23.0487C23.5437 19.489 24.0226 19.4274 24.4852 19.304C24.9479 19.1805 25.382 18.8898 25.7879 18.4317C26.1207 18.0704 26.3803 17.6652 26.567 17.2159C26.7618 16.7665 26.8877 16.3085 26.9444 15.8414C27.0905 14.8193 26.9486 14.0132 26.5183 13.4229C26.0881 12.8327 25.3942 12.533 24.4365 12.5243ZM22.5958 14.0441L21.8678 17.9956C21.9165 18.0043 21.9652 18.0088 22.0139 18.0088H22.1843C22.9635 18.0176 23.6128 17.934 24.1322 17.7577C24.6515 17.5727 25.0007 16.9296 25.1792 15.8282C25.3253 14.9031 25.1792 14.37 24.7409 14.2291C24.3107 14.0882 23.7711 14.0221 23.1217 14.0309C23.0243 14.0396 22.9311 14.0441 22.8417 14.0441H22.5958Z" fill="white"></path> </g></svg>
            ),
        },
        {
            name: "JavaScript",
            desc: t("skills13"),
            value: 80,
            color: "#F7DF1E",
            svg: (
                <svg viewBox="0 0 32 32" fill="none" width="25" height="25" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <rect x="2" y="2" width="28" height="28" fill="#FFCA28"></rect> <path d="M19 25.2879L21.0615 23.9237C21.2231 24.4313 22.2462 25.6368 23.5385 25.6368C24.8308 25.6368 25.4308 24.931 25.4308 24.463C25.4308 23.1878 24.1112 22.7382 23.4774 22.5223C23.374 22.4871 23.289 22.4581 23.2308 22.4328C23.2009 22.4198 23.1558 22.4025 23.0979 22.3804C22.393 22.1111 19.7923 21.1175 19.7923 18.2373C19.7923 15.065 22.8538 14.7002 23.5462 14.7002C23.9991 14.7002 26.1769 14.7557 27.2615 16.7939L25.2615 18.1898C24.8231 17.3015 24.0946 17.0081 23.6462 17.0081C22.5385 17.0081 22.3077 17.8201 22.3077 18.1898C22.3077 19.227 23.5112 19.6919 24.5273 20.0844C24.7932 20.1871 25.0462 20.2848 25.2615 20.3866C26.3692 20.91 28 21.7666 28 24.463C28 25.8136 26.8672 28.0002 24.0154 28.0002C20.1846 28.0002 19.1692 25.7003 19 25.2879Z" fill="#3E3E3E"></path> <path d="M9 25.5587L11.1487 24.1953C11.317 24.7026 11.9713 25.638 12.9205 25.638C13.8698 25.638 14.3557 24.663 14.3557 24.1953V15.0002H16.9982V24.1953C17.041 25.4636 16.3376 28.0002 13.2332 28.0002C10.379 28.0002 9.19242 26.3039 9 25.5587Z" fill="#3E3E3E"></path> </g></svg>
            ),
        },
        {
            name: "React",
            desc: t("skills14"),
            value: 88,
            color: "#61DAFB",
            svg: (
                <svg viewBox="0 0 32 32" fill="none" width="25" height="25" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M18.6789 15.9759C18.6789 14.5415 17.4796 13.3785 16 13.3785C14.5206 13.3785 13.3211 14.5415 13.3211 15.9759C13.3211 17.4105 14.5206 18.5734 16 18.5734C17.4796 18.5734 18.6789 17.4105 18.6789 15.9759Z" fill="#53C1DE"></path> <path fillRule="evenodd" clipRule="evenodd" d="M24.7004 11.1537C25.2661 8.92478 25.9772 4.79148 23.4704 3.39016C20.9753 1.99495 17.7284 4.66843 16.0139 6.27318C14.3044 4.68442 10.9663 2.02237 8.46163 3.42814C5.96751 4.82803 6.73664 8.8928 7.3149 11.1357C4.98831 11.7764 1 13.1564 1 15.9759C1 18.7874 4.98416 20.2888 7.29698 20.9289C6.71658 23.1842 5.98596 27.1909 8.48327 28.5877C10.9973 29.9932 14.325 27.3945 16.0554 25.7722C17.7809 27.3864 20.9966 30.0021 23.4922 28.6014C25.9956 27.1963 25.3436 23.1184 24.7653 20.8625C27.0073 20.221 31 18.7523 31 15.9759C31 13.1835 26.9903 11.7923 24.7004 11.1537ZM24.4162 19.667C24.0365 18.5016 23.524 17.2623 22.8971 15.9821C23.4955 14.7321 23.9881 13.5088 24.3572 12.3509C26.0359 12.8228 29.7185 13.9013 29.7185 15.9759C29.7185 18.07 26.1846 19.1587 24.4162 19.667ZM22.85 27.526C20.988 28.571 18.2221 26.0696 16.9478 24.8809C17.7932 23.9844 18.638 22.9422 19.4625 21.7849C20.9129 21.6602 22.283 21.4562 23.5256 21.1777C23.9326 22.7734 24.7202 26.4763 22.85 27.526ZM9.12362 27.5111C7.26143 26.47 8.11258 22.8946 8.53957 21.2333C9.76834 21.4969 11.1286 21.6865 12.5824 21.8008C13.4123 22.9332 14.2816 23.9741 15.1576 24.8857C14.0753 25.9008 10.9945 28.557 9.12362 27.5111ZM2.28149 15.9759C2.28149 13.874 5.94207 12.8033 7.65904 12.3326C8.03451 13.5165 8.52695 14.7544 9.12123 16.0062C8.51925 17.2766 8.01977 18.5341 7.64085 19.732C6.00369 19.2776 2.28149 18.0791 2.28149 15.9759ZM9.1037 4.50354C10.9735 3.45416 13.8747 6.00983 15.1159 7.16013C14.2444 8.06754 13.3831 9.1006 12.5603 10.2265C11.1494 10.3533 9.79875 10.5569 8.55709 10.8297C8.09125 9.02071 7.23592 5.55179 9.1037 4.50354ZM20.3793 11.5771C21.3365 11.6942 22.2536 11.85 23.1147 12.0406C22.8562 12.844 22.534 13.6841 22.1545 14.5453C21.6044 13.5333 21.0139 12.5416 20.3793 11.5771ZM16.0143 8.0481C16.6054 8.66897 17.1974 9.3623 17.7798 10.1145C16.5985 10.0603 15.4153 10.0601 14.234 10.1137C14.8169 9.36848 15.414 8.67618 16.0143 8.0481ZM9.8565 14.5444C9.48329 13.6862 9.16398 12.8424 8.90322 12.0275C9.75918 11.8418 10.672 11.69 11.623 11.5748C10.9866 12.5372 10.3971 13.5285 9.8565 14.5444ZM11.6503 20.4657C10.6679 20.3594 9.74126 20.2153 8.88556 20.0347C9.15044 19.2055 9.47678 18.3435 9.85796 17.4668C10.406 18.4933 11.0045 19.4942 11.6503 20.4657ZM16.0498 23.9915C15.4424 23.356 14.8365 22.6531 14.2448 21.8971C15.4328 21.9423 16.6231 21.9424 17.811 21.891C17.2268 22.6608 16.6369 23.3647 16.0498 23.9915ZM22.1667 17.4222C22.5677 18.3084 22.9057 19.1657 23.1742 19.9809C22.3043 20.1734 21.3652 20.3284 20.3757 20.4435C21.015 19.4607 21.6149 18.4536 22.1667 17.4222ZM18.7473 20.5941C16.9301 20.72 15.1016 20.7186 13.2838 20.6044C12.2509 19.1415 11.3314 17.603 10.5377 16.0058C11.3276 14.4119 12.2404 12.8764 13.2684 11.4158C15.0875 11.2825 16.9178 11.2821 18.7369 11.4166C19.7561 12.8771 20.6675 14.4086 21.4757 15.9881C20.6771 17.5812 19.7595 19.1198 18.7473 20.5941ZM22.8303 4.4666C24.7006 5.51254 23.8681 9.22726 23.4595 10.8426C22.2149 10.5641 20.8633 10.3569 19.4483 10.2281C18.6239 9.09004 17.7698 8.05518 16.9124 7.15949C18.1695 5.98441 20.9781 3.43089 22.8303 4.4666Z" fill="#53C1DE"></path> </g></svg>
            ),
        },
        {
            name: "MySQL",
            desc: t("skills15"),
            value: 60,
            color: "#4479A1",
            svg: (
                <svg className="icon" viewBox="0 0 128 128">
                    <ellipse cx="64" cy="64" rx="52" ry="36" fill="#4479A1" />
                    <text x="64" y="80" textAnchor="middle" fontSize="28" fill="#fff">MySQL</text>
                </svg>
            ),
        },
        {
            name: "GIMP",
            desc: t("skills16"),
            value: 95,
            color: "#544E4E",
            svg: (
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
                    <radialGradient id="LoYOxvrky7jpmBj9eYIMBa_omuSvqfyybfC_gr1" cx="22.415" cy="20.182" r="17.612" gradientTransform="matrix(1 0 0 .9978 0 .009)" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#a69d88"></stop><stop offset="1" stopColor="#3e3a2e"></stop></radialGradient><path fill="url(#LoYOxvrky7jpmBj9eYIMBa_omuSvqfyybfC_gr1)" d="M8,20.962c0,0,2-0.998,2-2.993s0-9.978,0-9.978s3.959,8.098,14.037,6.54	c9.46-1.462,13.385-4.982,15.5-10.264c0.142-0.356,0.66-0.307,0.722,0.071C41.28,10.564,43.23,33.274,26,33.933	c0,0-10,0.998-19-6.984C8,24.953,8,20.962,8,20.962z"></path><radialGradient id="LoYOxvrky7jpmBj9eYIMBb_omuSvqfyybfC_gr2" cx="4.477" cy="18.886" r="5.596" gradientTransform="matrix(.8717 -.5701 .5586 .8542 -9.83 6.704)" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#7d8587"></stop><stop offset="1" stopColor="#323538"></stop></radialGradient><path fill="url(#LoYOxvrky7jpmBj9eYIMBb_omuSvqfyybfC_gr2)" d="M9.375,19.825c2.009,3.072,1.881,6.71-0.286,8.127s-5.551,0.075-7.56-2.996	s-1.881-6.71,0.286-8.127S7.366,16.754,9.375,19.825z"></path><radialGradient id="LoYOxvrky7jpmBj9eYIMBc_omuSvqfyybfC_gr3" cx="23.16" cy="15.649" r="5.755" gradientTransform="matrix(.9876 0 0 1 .372 0)" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#fafafb"></stop><stop offset="1" stopColor="#c8cdd1"></stop></radialGradient><ellipse cx="24.074" cy="17.5" fill="url(#LoYOxvrky7jpmBj9eYIMBc_omuSvqfyybfC_gr3)" rx="5.926" ry="5.5"></ellipse><radialGradient id="LoYOxvrky7jpmBj9eYIMBd_omuSvqfyybfC_gr4" cx="13.213" cy="17.287" r="3.646" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#fafafb"></stop><stop offset="1" stopColor="#c8cdd1"></stop></radialGradient><ellipse cx="13.787" cy="18.5" fill="url(#LoYOxvrky7jpmBj9eYIMBd_omuSvqfyybfC_gr4)" rx="3.787" ry="3.5"></ellipse><circle cx="25.5" cy="18.5" r="2.5" fill="#999"></circle><circle cx="25.5" cy="18.5" r="1.5" fill="#323537"></circle><circle cx="14.545" cy="19.168" r="1.737" fill="#999"></circle><circle cx="14.545" cy="19.168" r=".827" fill="#323537"></circle><circle cx="13.466" cy="18.041" r="1.11" fill="#fff"></circle><linearGradient id="LoYOxvrky7jpmBj9eYIMBe_omuSvqfyybfC_gr5" x1="2.266" x2="4.182" y1="17.558" y2="19.898" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#caced2"></stop><stop offset="1" stopColor="#8f979e"></stop></linearGradient><circle cx="3.543" cy="19.118" r="1.882" fill="url(#LoYOxvrky7jpmBj9eYIMBe_omuSvqfyybfC_gr5)"></circle><circle cx="23.761" cy="16.894" r="1.665" fill="#fff"></circle><linearGradient id="LoYOxvrky7jpmBj9eYIMBf_omuSvqfyybfC_gr6" x1="38.829" x2="36.996" y1="9.327" y2="16.812" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#77705f"></stop><stop offset="1" stopColor="#4b473a"></stop></linearGradient><path fill="url(#LoYOxvrky7jpmBj9eYIMBf_omuSvqfyybfC_gr6)" d="M39.883,4.932c0,0-1.149,4.452-2.872,6.798s-1.149,5.074,0.574,5.457	c1.723,0.383,0.91-0.766,0.239-1.005c-0.67-0.239-1.532-2.011-0.191-3.973C38.973,10.246,39.644,7.039,39.883,4.932z"></path><path fill="#181612" d="M31.697,28.485c0,0,0-2.298-2.394-2.489c0,0,0.748,0.299,1.2,0.984	c-3.043,2.676-8.907,1.602-8.907,1.602c5.074,1.394,8.234,0.157,9.257-0.367C31.323,28.716,31.697,28.485,31.697,28.485z"></path><linearGradient id="LoYOxvrky7jpmBj9eYIMBg_omuSvqfyybfC_gr7" x1="34.596" x2="33.202" y1="30.031" y2="32.079" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#c26715"></stop><stop offset=".508" stopColor="#b85515"></stop><stop offset="1" stopColor="#ad3f16"></stop></linearGradient><path fill="url(#LoYOxvrky7jpmBj9eYIMBg_omuSvqfyybfC_gr7)" d="M30.644,27.672c1.532,0.718,8.234,5.314,8.234,5.314s-0.862,1.005-1.58,2.011	c-1.819-0.957-7.915-6.223-7.915-6.223C29.287,28.294,30.053,27.432,30.644,27.672z"></path><linearGradient id="LoYOxvrky7jpmBj9eYIMBh_omuSvqfyybfC_gr8" x1="42.047" x2="40.275" y1="33.728" y2="36.112" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#fafafb"></stop><stop offset="1" stopColor="#c8cdd1"></stop></linearGradient><path fill="url(#LoYOxvrky7jpmBj9eYIMBh_omuSvqfyybfC_gr8)" d="M43.832,37.294l-4.955-4.309c-0.622-0.527-2.106,1.676-1.58,2.011l5.29,3.399L43.832,37.294z"></path><linearGradient id="LoYOxvrky7jpmBj9eYIMBi_omuSvqfyybfC_gr9" x1="46.488" x2="43.937" y1="40.911" y2="42.684" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#6d7479"></stop><stop offset="1" stopColor="#323538"></stop></linearGradient><path fill="url(#LoYOxvrky7jpmBj9eYIMBi_omuSvqfyybfC_gr9)" d="M48,45.576c0,0-6.234-1.787-5.677-6.846c0.128-1.161,1.349-1.412,1.349-1.412	s2.978-0.694,3.257,3.614C47.116,43.805,48,45.576,48,45.576z"></path>
                </svg>
            ),
        },
        {
            name: "Photoshop",
            desc: t("skills17"),
            value: 95,
            color: "#31A8FF",
            svg: (
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
                    <path fill="#03A9F4" d="M6,10c0-2.209,1.791-4,4-4h28c2.209,0,4,1.791,4,4v28c0,2.209-1.791,4-4,4H10c-2.209,0-4-1.791-4-4V10z"></path><path fill="#020F16" d="M20.016,19.174h-2.002v4.434h1.973c0.547,0,0.97-0.179,1.27-0.537s0.449-0.879,0.449-1.563c0-0.71-0.153-1.274-0.459-1.694S20.53,19.181,20.016,19.174z"></path><path fill="#020F16" d="M9,9v30h30V9H9z M23.365,24.789C22.539,25.597,21.393,26,19.928,26h-1.914v5h-2.871V16.781h4.844c1.406,0,2.528,0.437,3.364,1.309s1.255,2.005,1.255,3.398S24.192,23.981,23.365,24.789z M32.682,30.336c-0.709,0.573-1.641,0.859-2.793,0.859c-0.775,0-1.459-0.151-2.051-0.454s-1.057-0.725-1.392-1.265s-0.503-1.123-0.503-1.748h2.627c0.014,0.481,0.125,0.843,0.337,1.084s0.558,0.361,1.04,0.361c0.742,0,1.113-0.335,1.113-1.006c0-0.234-0.112-0.451-0.337-0.649S30,27.052,29.225,26.713c-1.139-0.462-1.922-0.94-2.349-1.436s-0.64-1.11-0.64-1.846c0-0.925,0.334-1.688,1.001-2.29s1.552-0.903,2.651-0.903c1.158,0,2.086,0.3,2.783,0.898s1.045,1.403,1.045,2.412h-2.764c0-0.859-0.357-1.289-1.074-1.289c-0.293,0-0.533,0.091-0.723,0.273s-0.283,0.437-0.283,0.762c0,0.234,0.104,0.441,0.313,0.62s0.699,0.435,1.475,0.767c1.127,0.417,1.922,0.881,2.388,1.392s0.698,1.174,0.698,1.987C33.746,29.005,33.391,29.763,32.682,30.336z"></path>
                </svg>
            ),
        },
        {
            name: "Figma",
            desc: t("skills18"),
            value: 70,
            color: "#F24E1E",
            svg: (
                <svg className="icon" viewBox="0 0 128 128" aria-hidden="true">
                    <circle cx="64" cy="30" r="15" fill="#F24E1E" />
                    <rect x="49" y="45" width="30" height="30" rx="15" fill="#A259FF" />
                    <rect x="49" y="80" width="30" height="30" rx="15" fill="#1ABCFE" />
                    <rect x="64" y="65" width="15" height="15" rx="7.5" fill="#0ACF83" />
                </svg>
            ),
        },
    ];


    const skillsRef = useRef(null);
    const [loaded, setLoaded] = useState(false);

    useEffect(() => {
        const observer = new IntersectionObserver(
            ([entry]) => {
                if (entry.isIntersecting) {
                    setLoaded(true);
                    observer.disconnect();
                }
            },
            { threshold: 0.3 } // la sezione deve essere almeno 30% visibile
        );

        if (skillsRef.current) {
            observer.observe(skillsRef.current);
        }

        return () => observer.disconnect();
    }, []);

    return (
        <main className="container">
            <section className="section" aria-labelledby="skills-heading">
                <div className="header">
                    <h2 className="text-white text-left display-5 text-uppercase">
                        {t('skills1')}
                    </h2>
                </div>

                <div className="grid" role="list" ref={skillsRef}>
                    {skills.map((skill) => (
                        <article
                            key={skill.name}
                            className="card"
                            role="listitem"
                            tabIndex="0"
                            aria-label={skill.name}
                        >
                            <span className="chip" title={skill.name}>
                                {skill.svg}
                                {skill.name}
                            </span>
                            <p className="desc">{skill.desc}</p>
                            <div className="meter" aria-hidden="true">
                                <span
                                    style={{
                                        width: loaded ? `${skill.value}%` : "0%",
                                        backgroundColor: skill.color,
                                        transition: "width 1.5s ease-in-out",
                                    }}
                                />
                            </div>
                        </article>
                    ))}
                </div>

            </section>
        </main>
    );
}

